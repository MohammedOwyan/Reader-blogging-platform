...
import { faBold, faImage, faItalic, faLink, faList, faList12, faStrikethrough, faUnderline } from '@fortawesome/free-solid-svg-icons';
        ...
        <div >
            <nav className="flex bg-white flex-row justify-between items-center px-60 py-5 ">
                ...
                <div className="flex justify-between items-center w-60 ">
                    ...
                    <Avatar className='size-11 '>
                        <AvatarImage src="#" />
                        <AvatarFallback className='bg-gray-400 text-white'>MI</AvatarFallback>
                    </Avatar>
                </div>
            </nav>
            <div className="container flex flex-col items-center mx-auto mt-10 w-2/3">
                <div className="flex justify-between w-full py-4">
                    ...
                    <Toggle className={`${editor.isActive("bold") ? "!bg-black   !text-white" : ""}`}
                        onClick={() => editor.chain().focus().toggleBold().run()}>
                        <FontAwesomeIcon icon={faBold} />
                    </Toggle>
                    <Toggle className={`${editor.isActive("italic") ? "!bg-black   !text-white" : ""}`}
                        onClick={() => editor.chain().focus().toggleItalic().run()}>
                        <FontAwesomeIcon icon={faItalic} />
                    </Toggle>
                    <Toggle className={`${editor.isActive("underline") ? "!bg-black   !text-white" : ""}`}
                        onClick={() => editor.chain().focus().toggleUnderline().run()}>
                        <FontAwesomeIcon icon={faUnderline} />
                    </Toggle>
                    <Toggle className={`${editor.isActive("strike") ? "!bg-black   !text-white" : ""}`}
                        onClick={() => editor.chain().focus().toggleStrike().run()}>
                        <FontAwesomeIcon icon={faStrikethrough} />
                    </Toggle>
                    {/* <button onClick={() => editor.chain().focus().toggleHeading({ level: 1 }).run()}>H1</button> */}
                    <Toggle className={`${editor.isActive("bulletList") ? "!bg-black   !text-white" : ""}`}
                        onClick={() => editor.chain().focus().toggleBulletList().run()}>
                        <FontAwesomeIcon icon={faList} />
                    </Toggle>
                    <Toggle className={`${editor.isActive("orderedList") ? "!bg-black   !text-white" : ""}`}
                        onClick={() => editor.chain().focus().toggleOrderedList().run()}>
                        <FontAwesomeIcon icon={faList12} />
                    </Toggle>
                    <Toggle className={`${editor.isActive("link") ? "!bg-black   !text-white" : ""}`}
                ...
                />
                <EditorContent className='text-lg mt-6 border-t-2 pt-5 self-start w-full' editor={editor} />
            </div>
        </div>
    );
};
